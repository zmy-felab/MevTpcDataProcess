...
void ADCcode_correct_cali(std::string correct_cali_dictionary)
{   
    // Set data path
    std::string raw_data_dictionary = correct_cali_dictionary + "raw_data/";
    std::string root_data_dictionary = correct_cali_dictionary + "root_data/";

    std::cout << "Start ADC code calibration with data files at path:" << raw_data_dictionary << std::endl << "\n" << std::endl;

    // read raw data to root data
    raw_data_read_1v0(correct_cali_dictionary);
